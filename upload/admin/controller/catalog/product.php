...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			if ($this->model_tool_image->is_fileimg($result['image'])) {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['image']) && $this->model_tool_image->is_fileimg($this->request->post['image'])) {
			$data['thumb'] = $this->model_tool_image->resize($this->request->post['image'], 100, 100);
		//} elseif (!empty($product_info) && is_file(DIR_IMAGE . $product_info['image'])) {
		} elseif (!empty($product_info) && $this->model_tool_image->is_fileimg($product_info['image'])) {
		...
		foreach ($product_images as $product_image) {
			...
			if ($this->model_tool_image->is_fileimg($product_image['image'])) {
